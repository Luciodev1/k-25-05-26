...
def _update_export_job(job_id: int | None, status: str, **kwargs) -> None:
    if job_id is None:
        return
    try:
        ...
        if status == 'processing':
            ...
            job.mark_completed(kwargs.get('file_path', ''))
        elif status == 'failed':
            job.mark_failed(kwargs.get('error_message', 'Erro desconhecido'))
    except ExportJob.DoesNotExist:
        logger.warning('ExportJob %s not found for status update', job_id)


def _wrap_task_result(result: dict[str, Any] | None, job_id: int | None) -> dict[str, Any] | None:
...
def async_outflows_by_customer_report(
    ...
    _update_export_job(job_id, 'processing')
    try:
        ...
        _update_export_job(job_id, 'completed', file_path=path)
        return _wrap_task_result(result, job_id)
    except Exception as exc:
        logger.error('Export async failed: %s', exc)
        _update_export_job(job_id, 'failed', error_message=str(exc))
        raise
